...
export const InviteModal = () => {
    ...
    //check to see if modal is open and user is inviting people
    const isModalOpen = isOpen && type === "invite";

    
    const { server } = data;
    //create an invite url of [origin]/invite/[server's invite code]
    const inviteUrl = `${origin}/invite/${server?.inviteCode}`
    
    //create an oncopy event handler here (arrow function)
    const onCopy = () => {
    ...
    //create a async function that generates a new link and refreshes code
    const onNew =  async () => {
        try{
            setIsLoading(true);
    ...
    return (
        ...
        <Dialog open={isModalOpen} onOpenChange={onClose}>
            ...
            >
                ...
                <div className="p-6">
                    <Label 
                    className="uppercase text-sx font-bold text-zinc-500 dark:text-[#646D6E]"
                    >
                        Server Invite Link
                    </Label>
                    <div className="flex items-center mt-2 gap-x-2">
                        ...
                        <Button 
                            ...
                            className="dark:bg-zinc-900 cursor-pointer">
                                {copied 
                            ? <Check className="w-4 h-4 dark:text-green-600" /> 
                    ...
                    <Button
                        ...
                        variant="link"
                        size="sm"
                        className="text-xs text-zinc-500 dark:text-white mt-4 cursor-pointer"
